...
export default function AdminDashboard() {
  ...
  const [modal, setModal] = useState(null); // 'patient' | 'nurse' | null
  ...
  const loadAll = useCallback(async () => {
    setLoading(true);
    const [p, n, pn, d, a] = await Promise.all([
      loadPatients(),
      loadNurses(),
      loadPendingNurses(),
      loadDoctors(),
  ...
  async function handleApprove(nurse) {
    try {
      ...
      showToast('Nurse Approved', `Nurse ${nurse.profiles?.name || ''} has been verified and activated.`);
      loadAll();
    } catch (err) {
  ...
  async function handleDeletePatient(patientId) {
    if (!confirm('Are you sure you want to remove this patient from monitoring?')) return;
    const { error } = await supabase.from('patients').delete().eq('id', patientId);
    if (error) {
      alert(error.message);
      return;
    }
    showToast('Patient Removed', 'Patient deleted from system.');
    loadAll();
  }

  async function handleDeleteNurse(nurseId) {
    if (!confirm('Are you sure you want to remove this nurse from the system?')) return;
    const { error } = await supabase.from('nurses').delete().eq('id', nurseId);
    if (error) {
      alert(error.message);
      return;
    }
    showToast('Nurse Removed', 'Nurse deleted from system.');
    loadAll();
  }
    ...
    >
      ...
      {active === 'overview' && (
        <Overview
          ...
          loading={loading}
          onManagePatients={() => setActive('patients')}
          onManageNurses={() => setActive('nurses')}
        />
      )}
      ...
      {active === 'nurses' && (
        <NursesSection 
          nurses={nurses} 
          loading={loading} 
          onAdd={() => setModal('nurse')}
          onEdit={(nurse) => setEditingNurse(nurse)}
          onDelete={handleDeleteNurse} 
        />
      )}

      {/* Nurse Verification Pending Requests Section */}
      {active === 'verification' && (
          ...
          <div className="flex items-center justify-between bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                Review and verify nurse registration requests before enabling system access.
              </p>
            </div>
      ...
      <EditNurseModal 
        ...
        onDone={() => { setEditingNurse(null); loadAll(); }} 
        showToast={showToast} 
      />
...
function sectionTitle(key) {
  return { 
    ...
    reports: 'Telemetry & Patient Reports'
  }[key];
}
function sectionSubtitle(key) {
  return {
    ...
    messages: 'Send announcements and direct messages to nurses & staff.',
    reports: 'Export patient records and IV telemetry as CSV or PDF report.'
  }[key];
}
